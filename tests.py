...
class SirTests(unittest.TestCase):
    def test_small_beta(self):
        ...
        actual_under_beta = sir.model_approx(model_init, experimantal,
            var_name = 'beta', var_init = 0, learning_rate = 0.0005, max_iter = 500, threshold = 0)
        actual_over_beta = sir.model_approx(model_init, experimantal,
            var_name = 'beta', var_init = 0.001, learning_rate = 0.00001, max_iter = 500)
    ...
    def test_susceptible_search(self):
        expected_susceptible = 750
        model_init = sir.Model(alpha = 0.5, beta = 0.3, s_init = expected_susceptible)
        experimantal = model_init.run()

        actual_under = sir.model_approx(model_init, experimantal,
            var_name = 's_init', var_init = 300, learning_rate = 2, max_iter = 300)
        actual_over = sir.model_approx(model_init, experimantal,
            var_name = 's_init', var_init = 1200, learning_rate = 2, max_iter = 300)
    ...
    def test_alpha_search(self):
        ...
        actual_under_alpha = sir.model_approx(model_init, experimantal,
            var_name = 'alpha', var_init = 0.05, learning_rate = 0.01, max_iter = 100)
        actual_over_alpha = sir.model_approx(model_init, experimantal,
            var_name = 'alpha', var_init = 1, learning_rate = 0.01, max_iter = 100)
    ...
    def test_beta_search(self):
        ...
        actual_under_beta = sir.model_approx(model_init, experimantal,
            var_name = 'beta', var_init = 0.1, learning_rate = 0.01, max_iter = 100)
        actual_over_beta = sir.model_approx(model_init, experimantal,
            var_name = 'beta', var_init = 1, learning_rate = 0.01, max_iter = 100)
